...
| `trainerNotifyNumber` | phone number or Apple ID email          | Yes (if mode ≠ reply) | —         | Handle to receive notifications (E.164 or `user@icloud.com`) |
...
A `senderIsOwner` guard is applied before wrapping: if the original sender matches `trainerNotifyNumber` (via `matchesHandle` — digits-only for phones, case-insensitive string for Apple ID emails), the interceptor is skipped entirely. This prevents owner messages processed by the agent from being re-captured as if they were agent output.
...
```
$OPENCLAW_STATE_DIR/imessage-trainer-pending-<accountId>.json
```

One file is created per iMessage account, keeping pending entries isolated in multi-account setups.
...
interface PendingMessage {
  ...
  content: string; // original message text (JSON-serialized in MEMORY.md)
  draft: string; // agent's captured reply draft (JSON-serialized in MEMORY.md)
...
| `workspaceDir` | `OPENCLAW_WORKSPACE_DIR` env | `config.agents.defaults.workspace` | `~/.openclaw/workspace-{profile}` or `workspace` |

The fallback mirrors `resolveDefaultAgentWorkspaceDir`: it is profile-aware, using `~/.openclaw/workspace-{OPENCLAW_PROFILE}` when a non-default profile is active and `~/.openclaw/workspace` otherwise. This ensures MEMORY.md is written to the same directory the agent session reads from with no additional configuration.
...
**76 tests across 12 suites:**

| Suite                                                            | Cases                                                                                                                                                                                                   |
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `parseOwnerReply`                                                | send/custom/unknown, case normalisation, bracket variants, edge cases                                                                                                                                   |
| `resolveTrainerStateDirs`                                        | env overrides, HOME fallback, missing HOME, config workspace fallback, env takes precedence over config, profile-aware workspace fallback                                                               |
...
| `matchesHandle`                                                  | phone matching with formatting differences, email handle case-insensitivity, cross-type mismatches                                                                                                      |
| `pending store per-account namespacing`                          | separate files written for different accountIds, entries isolated                                                                                                                                       |
| `MEMORY.md log format — special characters`                      | embedded quotes escaped, newlines kept on single log line                                                                                                                                               |
