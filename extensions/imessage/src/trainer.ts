...
/**
 * Returns true when two handles refer to the same contact.
 * For phone numbers, compares digits-only (handles formatting differences).
 * For Apple ID email handles (no digits), falls back to case-insensitive exact match.
 */
export function matchesHandle(a: string, b: string): boolean {
...
export function resolveTrainerStateDirs(
  ...
  const home = env.HOME ?? "";
  // Mirror the profile-aware fallback from resolveDefaultAgentWorkspaceDir so
  // MEMORY.md is written to the same workspace the agent session reads from.
  ...
  return {
    ...
    workspaceDir:
      env.OPENCLAW_WORKSPACE_DIR ?? cfg?.agents?.defaults?.workspace ?? defaultWorkspace,
  };
}

function getPendingStorePath(stateDir: string, accountId: string): string {
  // Sanitize accountId so it's safe as a filename component.
...
async function loadPendingStore(stateDir: string, accountId: string): Promise<PendingStore> {
  try {
    return parsePendingStore(
      JSON.parse(await readFile(getPendingStorePath(stateDir, accountId), "utf-8")),
    );
  } catch {
    return { messages: {}, lastUpdated: Date.now() };
  }
...
async function savePendingStore(
  stateDir: string,
  accountId: string,
  store: PendingStore,
): Promise<void> {
  const path = getPendingStorePath(stateDir, accountId);
...
async function addPending(
  ...
  const store = await loadPendingStore(stateDir, accountId);
  ...
  await savePendingStore(stateDir, accountId, store);
}

async function removePending(stateDir: string, accountId: string, shortId: string): Promise<void> {
  const store = await loadPendingStore(stateDir, accountId);
  delete store.messages[shortId];
  await savePendingStore(stateDir, accountId, store);
}

async function markPendingNotificationFailed(
  stateDir: string,
  accountId: string,
  shortId: string,
): Promise<void> {
...
export async function expireTrainerPending(params: {
  ...
  const store = await loadPendingStore(stateDir, account.accountId);
  const now = Date.now();
  let count = 0;
  let changed = false;
  ...
  if (changed) {
    await savePendingStore(stateDir, account.accountId, store);
  }
  return count;
}
...
export async function handleTrainerOwnerReply(params: {
  ...
  const verb =
    pending.mode === "supervised"
      ...
      : parsed.type === "send-draft"
        ? "approved"
        : "would have sent";
  // JSON.stringify the user-content fields so embedded quotes and newlines never
  // break the single-line log format (each entry must stay on one line).
  const logLine = `- [${timestamp}] From ${pending.fromName ?? pending.from}: ${JSON.stringify(pending.content)} | I drafted: ${JSON.stringify(pending.draft)} | Owner ${verb}: ${JSON.stringify(textToSend)}`;
  ...
  await removePending(stateDir, account.accountId, parsed.shortId);
...
export function createTrainerSendInterceptor(
  ...
  return async (to: string, text: string): Promise<void> => {
    ...
    try {
      await addPending(stateDir, accountId, {
        id: shortId,
        from: originalFrom,
    ...
    if (notifyErr) {
      ...
      await markPendingNotificationFailed(stateDir, accountId, shortId).catch((err) => {
        runtime.error?.(
