...
function pendingStorePath(stateDir: string, accountId = "default") {
  const safeId = accountId.replace(/[^a-zA-Z0-9_-]/g, "_");
  return join(stateDir, `imessage-trainer-pending-${safeId}.json`);
}
...
async function writePendingStore(
  ...
  await writeFile(
    pendingStorePath(stateDir, accountId),
    JSON.stringify({ messages, lastUpdated: Date.now() }, null, 2),
    "utf-8",
  );
}

async function readPendingStore(stateDir: string, accountId = "default") {
  try {
    return JSON.parse(await readFile(pendingStorePath(stateDir, accountId), "utf-8")) as {
...
describe("resolveTrainerStateDirs", () => {
  ...
  it("uses profile-scoped workspace when OPENCLAW_PROFILE is set", () => {
    const result = resolveTrainerStateDirs({ HOME: "/home/testuser", OPENCLAW_PROFILE: "work" });
    expect(result.workspaceDir).toBe("/home/testuser/.openclaw/workspace-work");
  });

  it("uses default workspace when OPENCLAW_PROFILE is 'default'", () => {
    const result = resolveTrainerStateDirs({
      HOME: "/home/testuser",
  ...
  it("OPENCLAW_WORKSPACE_DIR takes precedence over OPENCLAW_PROFILE", () => {
    const result = resolveTrainerStateDirs({
      HOME: "/home/testuser",
      OPENCLAW_PROFILE: "work",
      OPENCLAW_WORKSPACE_DIR: "/override/workspace",
    });
    expect(result.workspaceDir).toBe("/override/workspace");
  });
});
...
describe("matchesHandle", () => {
  it("matches identical phone numbers", () => {
    expect(matchesHandle("+12025550100", "+12025550100")).toBe(true);
  });

  it("matches phone numbers with different formatting", () => {
    expect(matchesHandle("+1 (202) 555-0100", "+12025550100")).toBe(true);
  });

  it("does not match different phone numbers", () => {
    expect(matchesHandle("+12025550100", "+12025550199")).toBe(false);
  });

  it("matches Apple ID email handles case-insensitively", () => {
    expect(matchesHandle("alice@icloud.com", "Alice@iCloud.com")).toBe(true);
  });

  it("does not match different email handles", () => {
    expect(matchesHandle("alice@icloud.com", "bob@icloud.com")).toBe(false);
  });

  it("falls back to string comparison when one side has no digits", () => {
    expect(matchesHandle("alice@icloud.com", "+12025550100")).toBe(false);
  });
});

describe("pending store per-account namespacing", () => {
  let stateDir: string;
  beforeEach(async () => {
    stateDir = await mkTmpDir();
  });
  ...
  it("writes separate files for different accountIds", async () => {
    const sendMock = await getSendMock();
    sendMock.mockResolvedValue({ messageId: "m", sentText: "" });

    const makeCtx = (accountId: string) =>
      createTrainerSendInterceptor(async () => {}, {
        mode: "supervised",
        notifyNumber: "+12025550100",
        accountId,
        config: makeConfig(),
        ...
        originalFrom: "+15559998888",
        originalContent: "hey",
        originalMessageId: "0000000000000000ABCD",
      });

    await makeCtx("accountA")("+15559998888", "Draft A");
    await makeCtx("accountB")("+15559998888", "Draft B");

    const storeA = await readPendingStore(stateDir, "accountA");
    const storeB = await readPendingStore(stateDir, "accountB");
    expect(Object.keys(storeA.messages)).toHaveLength(1);
    expect(Object.keys(storeB.messages)).toHaveLength(1);
    expect((Object.values(storeA.messages)[0] as Record<string, unknown>).draft).toBe("Draft A");
    expect((Object.values(storeB.messages)[0] as Record<string, unknown>).draft).toBe("Draft B");
  });
});

describe("MEMORY.md log format — special characters", () => {
  let stateDir: string;
  let workspaceDir: string;
  beforeEach(async () => {
    stateDir = await mkTmpDir();
  ...
  afterEach(async () => {
    await rmTmpDir(stateDir);
    await rmTmpDir(workspaceDir);
    vi.clearAllMocks();
  });

  it("escapes embedded double-quotes in log line", async () => {
    const sendMock = await getSendMock();
    sendMock.mockResolvedValue({ messageId: "m", sentText: "" });
    ...
    await handleTrainerOwnerReply({
      replyText: "send ABCD",
      account: makeAccount({ trainerMode: "training" }),
      config: makeConfig(),
      runtime: makeRuntime(),
      stateDir,
      workspaceDir,
    ...
    const memory = await readFile(join(workspaceDir, "MEMORY.md"), "utf-8");
    expect(memory).toContain('\\"hello\\"');
    expect(memory).toContain('\\"hi\\"');
    const lines = memory.split("\n").filter((l) => l.startsWith("- ["));
    expect(lines).toHaveLength(1);
  });

  it("keeps log entry on a single line even when message contains newlines", async () => {
    const sendMock = await getSendMock();
    sendMock.mockResolvedValue({ messageId: "m", sentText: "" });
